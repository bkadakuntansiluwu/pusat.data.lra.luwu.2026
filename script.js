...
// =========================================================================
// V5 INFRASTRUCTURE â€” SMART NETWORK, QUEUE, TOKEN, CPU DETECT, QUOTA DISPLAY
// =========================================================================

// --- V5: Quota State ---
let _quotaInfo = { used: 0, remaining: 19000, percent: 0 };

// --- V5: Request Queue (1 concurrent, anti-tumpukan) ---
let _requestQueue = Promise.resolve();
async function _enqueueRequest(fn) {
    _requestQueue = _requestQueue.then(fn).catch(function() {});
    return _requestQueue;
}

// --- V5: Auth helpers ---
function _authHeaders() {
    if (SESSION_TOKEN) return { token: SESSION_TOKEN };
    if (SECRET_KEY) return { secret_key: SECRET_KEY };
    return {};
}
function _authUrlParam() {
    if (SESSION_TOKEN) return 'token=' + encodeURIComponent(SESSION_TOKEN);
    if (SECRET_KEY) return 'secret_key=' + encodeURIComponent(SECRET_KEY);
    return '';
}

// --- V5: Smart Fetch (retry otomatis + exponential backoff) ---
async function _smartFetch(url, options, maxRetry) {
    if (!maxRetry && maxRetry !== 0) maxRetry = 2;
    var attempt = 0;
...
// --- V5: CPU Speed Detection (untuk cetak) ---
let _cpuYieldInterval = 15;
let _cpuSpeedDetected = false;
function _detectCpuSpeed() {
    if (_cpuSpeedDetected) return;
    _cpuSpeedDetected = true;
    try {
        var start = performance.now();
        var dummy = document.createElement('div');
        dummy.style.cssText = 'position:absolute;left:-9999px;top:-9999px;';
        dummy.innerHTML = 'x'.repeat(5000);
        document.body.appendChild(dummy);
        for (var i = 0; i < 50; i++) { dummy.getBoundingClientRect(); }
        document.body.removeChild(dummy);
        var elapsed = performance.now() - start;
        if (elapsed > 50) _cpuYieldInterval = 50;
        else if (elapsed > 25) _cpuYieldInterval = 35;
        else if (elapsed > 12) _cpuYieldInterval = 25;
    } catch(e) {}
}
...
function _updateQuotaDisplay(quota) {
    if (!quota) return;
    _quotaInfo = quota;
    var el = document.getElementById('quotaInfoBadge');
    if (!el) {
        var container = document.getElementById('statusKoneksi');
        if (container && container.parentElement) {
            el = document.createElement('span');
            el.id = 'quotaInfoBadge';
            el.style.cssText = 'font-size:10px; color:#64748b; margin-left:8px;';
            container.parentElement.appendChild(el);
        }
    }
    if (el) {
        var color = quota.percent > 80 ? '#ef4444' : quota.percent > 50 ? '#f59e0b' : '#22c55e';
        el.innerHTML = '<i class="fa-solid fa-gauge-high" style="color:' + color + ';"></i> Kuota: ' + quota.percent + '%';
        el.title = 'Terpakai: ' + quota.used + ' | Sisa: ' + quota.remaining;
    }
}
...
function syncTTDKeCloud() {
    ...
    Object.assign(payload, _authHeaders());
    
    _enqueueRequest(async function() {
        try {
            ...
            if (res._quota) _updateQuotaDisplay(res._quota);
...
function muatTTDDariCloud() {
    ...
    let tahun = document.getElementById('selectTahun').value;
    let authParam = _authUrlParam();
    let url = SCRIPT_URL_DATABASE + '?action=load_ttd&tahun=' + tahun + '&kode_skpd=' + kodeSkpdAktif + '&' + authParam;
    
    _enqueueRequest(async function() {
        try {
            let response = await _smartFetch(url, { cache: 'no-store' });
            let res = await response.json();
            if (res._quota) _updateQuotaDisplay(res._quota);
    ...
    Swal.fire({
        ...
        _enqueueRequest(async function() {
            try {
                ...
                Swal.fire('Error', 'Gagal koneksi: ' + err.message, 'error');
            }
        });
    });
}
...
async function cetakPro() { 
    ...
    while (queue.length > 0 && safetyCounter < SAFETY_MAX) {
        ...
        // [UPDATE] Setiap 10 baris, hentikan proses selama 15 milidetik.
        // Ini memberi waktu bagi CPU komputer lawas untuk "bernapas" 
        // dan mencegah browser mengalami 'Not Responding' atau 'Freeze'.
        if (safetyCounter % 10 === 0) {
            await new Promise(resolve => setTimeout(resolve, _cpuYieldInterval));
        }
...
async function simpanKeCloud() {
    return _enqueueRequest(async function() {
    ...
    for (let i = 0; i < totalChunks; i++) {
        ...
        try {
            ...
            Object.assign(chunkPayload, _authHeaders());
...
function eksekusiTarikDataLRA() {
    ...
    _enqueueRequest(async function() {
        try {
            let response = await _smartFetch(fetchUrl, { cache: 'no-store' });
            let res = await response.json();
            if (res._quota) _updateQuotaDisplay(res._quota);
...
function cekKoneksiServer() {
    ...
    _smartFetch(urlPing, { cache: 'no-store' }, 1).then(function(response) {
        return response.json();
    }).then(function(res) {
        if (res._quota) _updateQuotaDisplay(res._quota);
        
        if (res && res.status === 'success') {
            if (res.token && !SESSION_TOKEN) {
...
function fungsiKeluarAplikasi() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            sessionStorage.removeItem('LRA_SESSION_TOKEN');
...
function tampilkanGerbangKeamanan() {
    ...
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            // V5: Login via POST (password TIDAK muncul di URL)
            return fetch(SCRIPT_URL_DATABASE, {
                ...
                .then(res => {
                    ...
                    if (res.token) {
                        SESSION_TOKEN = res.token;
                        sessionStorage.setItem('LRA_SESSION_TOKEN', SESSION_TOKEN);
                        return { token: res.token, quota: res.quota };
                    }
                    SECRET_KEY = pwd;
                    sessionStorage.setItem('LRA_AUTH_KEY', pwd);
                    return { token: null, quota: res.quota };
                })
                .catch(error => {
        ...
        if (result.isConfirmed) {
            if (result.value && result.value.quota) {
                _updateQuotaDisplay(result.value.quota);
            }
...
function tarikDataSiluman(tampilkanLoading = false) {
    ...
    let tahun = document.getElementById('selectTahun').value;
    let authParam = _authUrlParam();
    let fetchUrl = SCRIPT_URL_DATABASE + '?action=load&tahun=' + tahun + '&kode_skpd=' + kodeSkpdAktif + '&warm=true&t=' + waktuSekarang + '&' + authParam;
    ...
    _smartFetch(fetchUrl, { cache: 'no-store' }, 1).then(function(response) {
        return response.json();
    }).then(function(res) {
        if (res._quota) _updateQuotaDisplay(res._quota);
...
// 3. MESIN UTAMA ANTI-TABRAKAN (Menggunakan Jitter / Waktu Acak)
function jalankanWekerSiluman() {
    // [UPDATE] Angka dasar 3 menit (180000 ms) + angka acak hingga 1 menit (60000 ms).
    // Hasilnya: Weker menyala setiap 3 - 4 menit sekali. Sangat menghemat kuota Google!
    let waktuAcak = 180000 + Math.floor(Math.random() * 60000);
    ...
    setTimeout(function() {
        ...
        // 3. Admin belum AFK lebih dari 5 menit (lamaDiam <= 300000 ms)
        if (kodeSkpdAktif !== "" && statusTabAktif && lamaDiam <= 300000) {
            tarikDataSiluman(false); 
        }

        // Putar kembali wekernya untuk siklus berikutnya
