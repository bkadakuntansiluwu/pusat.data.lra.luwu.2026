...
// === V5 INFRASTRUCTURE: Smart Fetch + Request Queue ===
...
function muatTTDDariCloud() {
    ...
    let tahun = document.getElementById('selectTahun').value;
    let url = `${SCRIPT_URL_DATABASE}?action=load_ttd&tahun=${tahun}&kode_skpd=${kodeSkpdAktif}&${_getAuthParam()}`;
...
function eksekusiTarikDataLRA() {
    ...
    let fetchUrl = `${SCRIPT_URL_DATABASE}?action=load&tahun=${tahun}&kode_skpd=${kodeSkpdAktif}&${_getAuthParam()}`;
...
function cekKoneksiServer() {
    ...
    let urlPing = `${SCRIPT_URL_DATABASE}?${_getAuthParam()}&action=ping`;
        ...
        .then(res => {
            if (res && res.status === 'success') {
                ...
                if (res.quota !== undefined) {
                    let quotaBadge = document.getElementById('quotaInfo');
                    if (quotaBadge) quotaBadge.innerText = 'Kuota: ' + res.quota;
                }
                if (res.token) {
...
function _upgradeToToken(oldKey) {
    ...
    document.querySelectorAll('.container-fluid, .print-page').forEach(el => { el.style.filter = 'none'; el.style.pointerEvents = 'auto'; });
    fetch(SCRIPT_URL_DATABASE, {
        method: 'POST',
...
function tampilkanGerbangKeamanan() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            let successText = 'Selamat bekerja!';
            if (result.value.quota !== undefined) {
                successText += ' (Kuota tersisa: ' + result.value.quota + ')';
            }
            Swal.fire({
                icon: 'success', title: 'Berhasil Login', text: successText, timer: 1500, showConfirmButton: false
            });
                        pulihkanDataSesiLokal()
            cekKoneksiServer();
        }
...
function tarikDataSiluman(tampilkanLoading = false) {
    ...
    let tahun = document.getElementById('selectTahun').value;
    let fetchUrl = `${SCRIPT_URL_DATABASE}?action=load&tahun=${tahun}&kode_skpd=${kodeSkpdAktif}&${_getAuthParam()}&warm=true&t=${waktuSekarang}`;
